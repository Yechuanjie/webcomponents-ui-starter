# 基于vue构建web-components组件库

#### 背景

目前公司前端以vue技术栈为主，且vue2 和 vue3 项目会长期共存。对于新项目，我们将会统一使用`vue3技术栈`，那么之前开发的vue2组件就无法在vue3项目中使用。

同时，如果需要开发一个vue组件，能够同时运行在新老系统之上，也会面临着比较多的兼容性问题和部分毁灭性的改动，除非同时开发和维护两个vue版本的组件，类似`ElementUI `和 `Element Plus`
